...
d3.csv('data/assignments.csv').then(_data => {
  ...
  d3.select('#easy-assignment-count').text(data.filter(d => d.difficulty == 'Easy').length);
  d3.select('#difficult-assignment-count').text(data.filter(d => d.difficulty == 'Difficult').length);
